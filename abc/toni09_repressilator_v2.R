...
while(t <= N.populations){
    ...
    while(i <= N.particles){
        ...
        while(distance >= epsilons[t]){
            ...
            cat('.')
            if(t == 1){
                theta_star_star <- c(runif(1, min = -2, max = 10), runif(1, min = 0, max = 10), runif(1, min = -5, max = 20), runif(1, min = 500, max = 2500))            
            ...
            if((NROW(dfs) == length(times)) && (is.na(dfs[1000,"m1"])==FALSE) ){
